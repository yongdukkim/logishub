...
public class CargoInfoActivity extends AppCompatActivity {
        ...
        public boolean handleMessage(Message msg) {
            ...
            switch(msg.what)
            {
                ...
                case Define.HANDLER_MESSAGE_ERROR:
                    new CustomAlertDialog(CargoInfoActivity.this, "알림\n\n" + mErrorMessage);
                    break;
    ...
    private void setupViewPager(ViewPager viewPager) {
        ...
        {
            if (mArrMenuList.get(i).getName() != null && mArrMenuList.get(i).getLevel().equals(Define.MENU_LEVEL3) && Func.checkStringNull(mArrMenuList.get(i).getParentId()).equals(mMenuOid) && Func.checkStringNull(mArrMenuList.get(i).getDescription()).equals(mServiceType)) {

                switch (mArrMenuList.get(i).getName()) {
                    case Define.MENU_NAME_FREIGHT_CARGO_LIST:
                        adapter.addFragment(new CargoListFragment(), "0", mArrMenuList.get(i).getName());
                        break;
                    case Define.MENU_NAME_FREIGHT_CARGO_ALLOCATION_LIST:
                        adapter.addFragment(new CargoAllocationListFragment(), "1", mArrMenuList.get(i).getName());
                        break;
                }
            }
        }
